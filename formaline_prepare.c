...
int main() {
  ...
  fprintf(outfile,"void formalineoutput(char *outdir) {\n");
  ...
  // strcat doesn't like dealing with static arrays. Make a copy first and fix fortran string length
  ...
  fprintf(outfile,"outfile=fopen(strcat(copy,\"/helloWorld.src.tar.gz\"),\"w\");\n");
  ...
  fprintf(outfile,"void formalineoutput_(char *outdir) {\n");
  fprintf(outfile,"\t formalineoutput(outdir);\n");
  fprintf(outfile,"}\n");
